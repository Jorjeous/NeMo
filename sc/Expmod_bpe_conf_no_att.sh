...
CONFIG_PATH="/ws/NeMo/examples/asr/conf/conformer/"
CONFIG_NAME="conformer_ctc_bpe.yaml"
OPTIM="novograd"
LR=0.05
BETAS=[0.9,0.98]
WEIGHT_DECAY=1e-3
...
#++model.loss.reduction=”mean_volume” \
...
unirun \
  ...
  --partition "${PARTITION}" \
  --time-limit ${TIME_LIMIT:-"40:00"} \
  --slurm-logs-dir "${EXP_DIR_CLUSTER}" \
  --after-job "${AFTER_JOB}" \
  --save-cmd-to=${EXP_NAME}_outout.txt \
  ${REMOTE_FLAG} --verbose
